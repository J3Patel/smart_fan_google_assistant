...
exports.login = functions.https.onRequest((request, response) => {
  if (request.method === 'GET') {
    functions.logger.log(`Requesting login page- ${request.query.code}`);
    response.send(`
    <html>
      ...
      <body>
        <form action="/login" method="post">
          <input type="hidden"
            name="responseurl" value="${request.query.responseurl}&code=${request.query.code}" />
...
exports.fakeauth = functions.https.onRequest((request, response) => {
  const responseurl = util.format('%s?state=%s&code=%s',
      decodeURIComponent(request.query.redirect_uri), request.query.state, 'xxxxxx');
  functions.logger.log(`Set redirect as ${responseurl}`);
  // return commonLogin(request, response);
  return response.redirect(
      `/login?responseurl=${responseurl}&code=xxxxxx`);
});
